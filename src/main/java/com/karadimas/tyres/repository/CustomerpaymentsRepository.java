...
public interface CustomerpaymentsRepository extends JpaRepository<Customerpayments, Long>, JpaSpecificationExecutor<Customerpayments> {
    ...
    @Query(
        value = "select distinct customerpayments from Customerpayments customerpayments left join fetch customerpayments.customers",
    ...
    @Query("select distinct customerpayments from Customerpayments customerpayments left join fetch customerpayments.customers")
    List<Customerpayments> findAllWithToOneRelationships();

    @Query(
        "select customerpayments from Customerpayments customerpayments left join fetch customerpayments.customers where customerpayments.id =:id"
    )
