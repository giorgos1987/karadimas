...
import com.karadimas.tyres.service.CustomerDetailsQueryService;
import com.karadimas.tyres.service.CustomerDetailsService;
...
/**
 * REST controller for managing {@link com.karadimas.tyres.domain.CustomerDetails}.
 */
@RestController
@RequestMapping("/api")
public class CustomerDetailsResource {
    ...
    private final CustomerDetailsQueryService customerDetailsQueryService;

    public CustomerDetailsResource(
        CustomerDetailsService customerDetailsService,
        CustomerDetailsRepository customerDetailsRepository,
        CustomerDetailsQueryService customerDetailsQueryService
    ) {
        this.customerDetailsService = customerDetailsService;
        this.customerDetailsRepository = customerDetailsRepository;
        this.customerDetailsQueryService = customerDetailsQueryService;
    }
    ...
    public ResponseEntity<CustomerDetails> createCustomerDetails(@RequestBody CustomerDetails customerDetails) throws URISyntaxException {
    ...
    public ResponseEntity<List<CustomerDetails>> getAllCustomerDetails(
        CustomerDetailsCriteria criteria,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to get CustomerDetails by criteria: {}", criteria.toString().replaceAll("[\n\r\t]", "_"));
        Page<CustomerDetails> page = customerDetailsQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /customer-details/count} : count all the customerDetails.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/customer-details/count")
    public ResponseEntity<Long> countCustomerDetails(CustomerDetailsCriteria criteria) {
        log.debug("REST request to count CustomerDetails by criteria: {}", criteria.toString().replaceAll("[\n\r\t]", "_"));
        return ResponseEntity.ok().body(customerDetailsQueryService.countByCriteria(criteria));
    }
