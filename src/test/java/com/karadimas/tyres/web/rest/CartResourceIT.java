...
class CartResourceIT {
    ...
    @Test
    @Transactional
    void getAllCartsByPaymentReferenceIsEqualToSomething() throws Exception {
        // Initialize the database
        cartRepository.saveAndFlush(cart);

        // Get all the cartList where paymentReference equals to DEFAULT_PAYMENT_REFERENCE
        defaultCartShouldBeFound("paymentReference.equals=" + DEFAULT_PAYMENT_REFERENCE);

        // Get all the cartList where paymentReference equals to UPDATED_PAYMENT_REFERENCE
        defaultCartShouldNotBeFound("paymentReference.equals=" + UPDATED_PAYMENT_REFERENCE);
    }

    @Test
    @Transactional
    void getAllCartsByPaymentReferenceIsInShouldWork() throws Exception {
        // Initialize the database
        cartRepository.saveAndFlush(cart);

        // Get all the cartList where paymentReference in DEFAULT_PAYMENT_REFERENCE or UPDATED_PAYMENT_REFERENCE
        defaultCartShouldBeFound("paymentReference.in=" + DEFAULT_PAYMENT_REFERENCE + "," + UPDATED_PAYMENT_REFERENCE);

        // Get all the cartList where paymentReference equals to UPDATED_PAYMENT_REFERENCE
        defaultCartShouldNotBeFound("paymentReference.in=" + UPDATED_PAYMENT_REFERENCE);
    }

    @Test
    @Transactional
    void getAllCartsByPaymentReferenceIsNullOrNotNull() throws Exception {
        // Initialize the database
        cartRepository.saveAndFlush(cart);

        // Get all the cartList where paymentReference is not null
        defaultCartShouldBeFound("paymentReference.specified=true");

        // Get all the cartList where paymentReference is null
        defaultCartShouldNotBeFound("paymentReference.specified=false");
    }

    @Test
    @Transactional
    void getAllCartsByPaymentReferenceContainsSomething() throws Exception {
        // Initialize the database
        cartRepository.saveAndFlush(cart);

        // Get all the cartList where paymentReference contains DEFAULT_PAYMENT_REFERENCE
        defaultCartShouldBeFound("paymentReference.contains=" + DEFAULT_PAYMENT_REFERENCE);

        // Get all the cartList where paymentReference contains UPDATED_PAYMENT_REFERENCE
        defaultCartShouldNotBeFound("paymentReference.contains=" + UPDATED_PAYMENT_REFERENCE);
    }

    @Test
    @Transactional
    void getAllCartsByPaymentReferenceNotContainsSomething() throws Exception {
        // Initialize the database
        cartRepository.saveAndFlush(cart);

        // Get all the cartList where paymentReference does not contain DEFAULT_PAYMENT_REFERENCE
        defaultCartShouldNotBeFound("paymentReference.doesNotContain=" + DEFAULT_PAYMENT_REFERENCE);

        // Get all the cartList where paymentReference does not contain UPDATED_PAYMENT_REFERENCE
        defaultCartShouldBeFound("paymentReference.doesNotContain=" + UPDATED_PAYMENT_REFERENCE);
    }
    ...
    @Test
    @Transactional
    void getAllCartsByCustomerDetailsIsEqualToSomething() throws Exception {
        CustomerDetails customerDetails;
        if (TestUtil.findAll(em, CustomerDetails.class).isEmpty()) {
            cartRepository.saveAndFlush(cart);
            customerDetails = CustomerDetailsResourceIT.createEntity(em);
        ...
        // Get all the cartList where customerDetails equals to customerDetailsId
        defaultCartShouldBeFound("customerDetailsId.equals=" + customerDetailsId);

        // Get all the cartList where customerDetails equals to (customerDetailsId + 1)
        defaultCartShouldNotBeFound("customerDetailsId.equals=" + (customerDetailsId + 1));
    }
